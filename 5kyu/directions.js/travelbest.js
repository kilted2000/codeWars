...
//It should work for random tests too: expected 1066 to equal 993

function chooseBestSum(t, k, ls) {
    if (ls.length < k) return null;
    if (ls.length === k) return ls.reduce((a, b) => a + b, 0);
    if (ls.length > k) {
        let max = 0;
        for (let i = 0; i < ls.length; i++) {
            const sum = ls[i] + chooseBestSum(t - ls[i], k - 1, ls.slice(i + 1));
            if (sum <= t && sum > max) max = sum;
        }
...
function chooseBestSum(t, k, ls) {
    ...
    function dfs(t, k, ls, sum, idx) {
        ...
        for (let i = idx; i < ls.length; i++) {
            if (sum + ls[i] <= t) {
                dfs(t, k - 1, ls, sum + ls[i], i + 1);
            }
        }
    }
    dfs(t, k, ls, 0, 0);
    return biggest === -1 ? null : biggest;
}
